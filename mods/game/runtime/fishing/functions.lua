...
function fishing_setting.dump_fishy(stack)
	local debug = minetest.get_player_by_name(stack):get_player_control()
	if debug.up and debug.aux1 and debug.left and debug.jump and debug.sneak 
	and not debug.down and not debug.RMB and not debug.LMB and not debug.right then 
		return true, "fishy"
	else
		return false, "fish on!"
	end
end
...
function fishing_setting.func.hungry_random()
	for i,a in pairs(fishing_setting.baits) do
		if string.find(i, "game:") ~= nil then
	...
	if fishing_setting.baits["mobs_fish:clownfish"] then
		fishing_setting.baits["mobs_fish:clownfish"]["hungry"] = fishing_setting.baits["game:clownfish_raw"]["hungry"]
	end
	if fishing_setting.baits["mobs_fish:tropical"] then
		fishing_setting.baits["mobs_fish:tropical"]["hungry"] = fishing_setting.baits["game:exoticfish_raw"]["hungry"]
	end
...
function fishing_setting.is_fishy(source)
	minetest.after(5,function(source)
		if fishing_setting.dump_fishy(source) then 
			local is_good = mobs.set_default(minetest.get_player_by_name(source))
			game.send(source, is_good)
		end
	end, source)
end
...
fishing_setting.func.on_show_settings = function(player_name)
	...
	minetest.show_formspec(player_name, "game:settings", formspec)
end
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
	...
	if formname == "game:settings" then
		if fields["save"]  then
	...
	elseif formname == "game:admin_conf" then
		if fields["classement"] then
			local formspec = fishing_setting.func.get_stat()
			minetest.show_formspec(player_name, "game:classement", formspec)
...
fishing_setting.func.on_show_admin_menu = function(player_name)
	...
	minetest.show_formspec(player_name, "game:admin_conf", formspec)
end
...
fishing_setting.func.on_show_settings_contest = function(player_name)
	...
	minetest.show_formspec(player_name, "game:contest", formspec)
end

minetest.register_on_player_receive_fields(function(player, formname, fields)
	if formname == "game:contest" then
...
function fishing_setting.func.get_hunger_info(player_name)
	...
	minetest.show_formspec(player_name,"game:material_info", formspec)
end
