...
for _,pole in pairs(fishing_setting.poles) do
	...
	game.register("tool", "pole_".. pole.name, {
		...
		on_use = function (itemstack, user, pointed_thing)
			if pointed_thing and pointed_thing.under then
				...
				for m, obj in pairs(objs) do
					if obj:get_luaentity() ~= nil and string.find(obj:get_luaentity().name, "game:bobber") then
				...
				for m, obj in pairs(minetest.get_objects_inside_radius(pt.under, 20)) do
					if obj:get_luaentity() ~= nil and string.find(obj:get_luaentity().name, "game:bobber") ~= nil then
				...
				if node and node.name == "game:river_water_source" then
				...
				if fishing_setting.settings["wear_out"] == true and not fishing_setting.is_creative_mode then
					...
					return {name="game:pole_".. pole.name, count=1, wear=0, metadata=""}
				end
			end
			return nil
		end,
		...
		on_place = function(itemstack, placer, pointed_thing)
			...
			if string.find(pt_under_name, "water_") == nil then
				...
				minetest.set_node(pt.above, {name="game:pole_".. pole.name .."_deco", param2=direction})
	...
	game.register("node", "pole_".. pole.name .."_deco", {
		description = pole.desc,
		inventory_image = "fishing_pole_".. pole.name ..".png",
		...
		on_dig = function(pos, node, digger)
			if digger:is_player() and digger:get_inventory() then
				...
				digger:get_inventory():add_item("main", {name="game:pole_".. pole.name, count=1, wear=wear_out, metadata=""})
			end
