...
-- WORM ITEM
--------------------------------------------------------------------------------------------
game.register("craftitem", "bait_worm", {
	description = fishing_setting.func.S("Worm"),
	groups = { fishing_bait=1 ,not_in_creative_inventory=1,tool=1},
	inventory_image = "fishing_bait_worm.png",
	on_place = function(itemstack, placer, pointed_thing)
		local pt = pointed_thing
		minetest.add_entity({x=pt.under.x, y=pt.under.y+0.6, z=pt.under.z}, "game:bait_worm_entity")
		itemstack:take_item()
		return itemstack
	end,
	on_drop = function(itemstack, dropper, pos)
		minetest.add_entity({x = pos.x, y = pos.y, z = pos.z}, "game:bait_worm_entity")
		itemstack:take_item()
		return itemstack
	end,
})
...
-- WORM MOB
--------------------------------------------------------------------------------------------
minetest.register_entity("game:bait_worm_entity", {
	hp_max = 300,
	damage_over_time = 1,
	...
	on_rightclick = function(self, clicker)
		if clicker:is_player() and clicker:get_inventory() then
			clicker:get_inventory():add_item("main", "game:bait_worm")
			self.object:remove()
		end
	end,
	-- AI :D
...
if fishing_setting.settings["new_worm_source"] == false then
		...
		after_dig_node = function (pos, oldnode, oldmetadata, digger)
				...
				if tool_in_use == "" or tool_in_use == "default:dirt" then
					if fishing_setting.settings["worm_is_mob"] == true then
						minetest.add_entity({x = pos.x, y = pos.y+0.4, z = pos.z}, "game:bait_worm_entity")
					else
						local inv = digger:get_inventory()
						if inv:room_for_item("main", {name="game:bait_worm", count=1, wear=0, metadata=""}) then
							inv:add_item("main", {name="game:bait_worm", count=1, wear=0, metadata=""})
						end
					end
				end
			end
	...
	local function hoe_on_use(itemstack, user, pointed_thing, uses)
		...
		if math.random(1, 100) < fishing_setting.settings["worm_chance"] then
			if fishing_setting.settings["worm_is_mob"] == true then
				minetest.add_entity({x=pt.under.x, y=pt.under.y+0.4, z=pt.under.z}, "game:bait_worm_entity")
			else
