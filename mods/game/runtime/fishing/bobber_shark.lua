...
-- bobber shark
game.register("node", "bobber_shark_box", {
	drawtype = "nodebox",
	node_box = {
		type = "fixed",
		fixed = {
...
local FISHING_BOBBER_SHARK_ENTITY={
	...
	textures = {"game:bobber_shark_box"},
	--			   {left ,bottom, front, right,  top ,  back}
...
--	WHEN RIGHTCLICKING THE BOBBER THE FOLLOWING HAPPENS (CLICK AT THE RIGHT TIME WHILE HOLDING A FISHING POLE)
	on_rightclick = function (self, clicker)
		...
		if string.find(item_name, "game:pole_") ~= nil then
			if player_name ~= self.owner then return end
			if self.prize ~= "" then
		...
		elseif item_name == "game:baitball_shark" then
			if not fishing_setting.is_creative_mode then
				inv:remove_item("main", "game:baitball_shark")
			end
			self.baitball = 20
			--addparticle
...
minetest.register_entity("game:bobber_shark_entity", FISHING_BOBBER_SHARK_ENTITY)
