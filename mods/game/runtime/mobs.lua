...
local mobs_bar = {
	...
	visual = "sprite",
	textures = {"game_snow.png"},
	visual_size = {x = 0.5, y = 0.3, z = 0.3},
	wielder = nil,
}

minetest.register_entity("game:bar", mobs_bar)
...
-- particle effects
function effect(pos, amount, texture, max_size, min_size)
...
function mobs.set_default(source)
	if not source then
		return ""
	end
	source:set_properties({
		visual = "mesh",
		mesh = "charactermt.b3d",
		collisionbox = {-0.3,-1.0,-0.3, 0.3,0.2,0.3},
		visual_size = {x=1, y=1},
		textures = {"test.png"},
	})
	source:set_physics_override(1,1.2,1)
	source:set_armor_groups({fleshy=110})
	return "You really are a nerd!"
end
...
function check_for_death(self)
	...
	-- add exp to players
	if self.hitters then
		for i = 1, #self.hitters do
			game.add_exp(self.hitters[i], (self.exp/#self.hitters), self.lvl, #self.hitters)
		end
	end
...
function within_limits(pos, radius)
	...
	if  (pos.x - radius) > -19000
	and (pos.x + radius) <  19000
	and (pos.y - radius) > -19000
	and (pos.y + radius) <  19000
	and (pos.z - radius) > -19000
	and (pos.z + radius) <  19000 then
...
function node_ok(pos, fallback)
	...
	fallback = fallback or "game:dirt"
...
minetest.register_entity(":"..name, {
	...
	blood_texture = def.blood_texture or "bubble.png^[colorize:#aa0000:200",
	...
	on_step = function(self, dtime)
		...
		elseif self.state == "walk" then
			...
			if self.fly
			and self.fly_in == "game:water_source"
			and not lp then
		...
		-- stop attacking if player or out of range or dead
		if dist > self.view_range
		or not self.attack
		or not self.attack:getpos()
		or self.attack:get_hp() <= 0 
		or game.stats.hp[self.attack:get_player_name()] == "dead" then
			...
			self.object:set_properties(self)
			-- attack player
			if min_player and game.stats.hp[min_player:get_player_name()] ~= "dead" then
				do_attack(self, min_player)
			else
		...
		if self.state == "attack" then self.nametag_color = "#ff0000" else self.nametag_color = "#ffffff" end
		self.object:set_properties(self)
	end,
	...
	on_punch = function(self, hitter, tflp, tool_capabilities, dir)
		...
		local damage = weapon:get_definition().damage or 0
		local h_name = hitter:get_player_name()
		--self.object:set_hp(hp - damage)
		self.hitter = h_name
		table.insert(self.hitters, h_name)
		if #self.hitters > 10 then
			table.remove(self.hitters, 1)
		end
		...
		local pos = self.object:getpos()
		local hp_pos = {x=pos.x,y=pos.y+1+(self.lvl/30),z=pos.z}
		game.hp_loss(hp_pos, damage)
		...
		and self.child == false
		and self.hitter ~= self.owner then
...
local c_obsidian = minetest.get_content_id("game:obsidian")
local c_brick = minetest.get_content_id("game:obsidianbrick")
...
mobs:register_mob("game:wartus", {
	...
	hp_max = 40,
	armor = 100,
	collisionbox = {-0.7, -0.01, -0.7, 0.7, 1.8, 0.7},
	visual_size = {x = 1, y = 1},
	visual = "mesh",
	mesh = "mobs_pumba.x",
	textures = {
...
-- cooked porkchop
minetest.register_craftitem("game:pork_cooked", {
...
minetest.register_craft({
	type = "cooking",
	output = "game:pork_cooked",
	recipe = "game:pork_raw",
	cooktime = 5,
})